...
A browser-based parametric builder for a **laser-cut** two-part Magic: The Gathering deck box:
a finger-jointed open-top **body** holding a vertical stack of (sleeved) cards and a
finger-jointed telescoping **cap** that slides down over it. Ten flat panels, nested onto your
sheet and exported as real-millimetre SVG cut files — the default Commander box fits a single
300 × 300 mm sheet of 1/8″ basswood ply.
...
pnpm test       # 37 geometry/layout/schema tests (never a globally-installed vp)
```
...
- **Joinery** — box joints everywhere: wide walls own the corner comb's material phase, narrow
  walls the complement; floor and cap-top tabs pass through the walls and finish flush with the
  outer faces. Finger width is a target; each edge gets the largest odd comb count that keeps
  segments ≥ 60 % of it.
...
- **Retrieval** — a thumb-notch arc in the body front and optional semicircular side recesses,
  both capped at the cap depth so the closed box stays solid-walled.
